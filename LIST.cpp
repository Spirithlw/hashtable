...
const int MAX_LENGTH_OF_NAME = 32;
...
bool list_verify( const list_t* list )
    ...
    if ( (list->data == list->next) || (list->next == list->prev) || (list->prev == list->data) )
        {
        return false;
        }
...
void list_dot_dump( const list_t* list, bool open = false )
    ...
    while ( phys != 0)
        {
        fprintf( dot, "LIST%lu[ shape = record, label = \" %ld | phys:%ld | { next:%ld | prev:%ld } \" ]; ", elem, *(list->data+phys),
...
void list_dump ( const list_t* list, const char s1[] = "", const char s2[] = "", int line = 0 )
    ...
    for (int i = 0; i < list->max_size+1; ++i)
        ...
        fprintf( dump, "%-10ld %-10ld %-10ld\n", list->data[i], list->next[i], list->prev[i] );
        }

    fclose(dump);
    }

bool list_create ( list_t* list, size_t size )
    {
    assert( list );
    assert( size > 0 );
...
size_t list_find_by_value( const list_t* list, elem_t elem)
    {
    assert( list_verify(list) );
    size_t ind = 0;
    size_t i = list->head ;
...
int main()
    {
    listdef(lst)
    list_create( &lst, default_size_list );
    list_head_insert( &lst, 10);
    list_head_insert( &lst, 11);
    list_append( &lst, 18);
    ...
    printf("%ld\n", list_insert_before( &lst, 4, 17) );
    printf("%ld\n", list_insert_after( &lst, 9, 25) );
    list_delete_tail( &lst );
    list_delete_after( &lst, 2 );
    printf("%ld", list_find_by_value( &lst, 423) );
    list_dump( &lst, for_listdump );
    list_dot_dump( &lst );
    list_delete( &lst );
    return 0;
    }